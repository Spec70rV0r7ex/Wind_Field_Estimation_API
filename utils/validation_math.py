...
def calculate_metrics(pred_speeds: np.ndarray, pred_dirs: np.ndarray, true_speeds: np.ndarray, true_dirs: np.ndarray) -> dict:
    ...
    dir_bias = float(np.degrees(np.arctan2(np.mean(np.sin(np.radians(pred_dirs - true_dirs))),np.mean(np.cos(np.radians(pred_dirs - true_dirs))))))
